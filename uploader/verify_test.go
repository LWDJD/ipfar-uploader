...
// disableFallbackGateways clears DefaultFallbackGateways for the duration
// of a test so that unit tests never make real external HTTP calls.
func disableFallbackGateways() func() {
	old := ipfar.DefaultFallbackGateways
	ipfar.DefaultFallbackGateways = nil
	return func() {
		ipfar.DefaultFallbackGateways = old
	}
}
...
func newMockGateway(t *testing.T, carBytes []byte) (*arweave.GatewayClient, string, func()) {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		// GET /tx/{txID} returns transaction JSON (data_size as string per Arweave spec)
		if strings.HasPrefix(r.URL.Path, "/tx/") {
			w.Header().Set("Content-Type", "application/json")
			fmt.Fprintf(w, `{"block_height":100,"data_size":"%d"}`, len(carBytes))
			return
		}
...
//
// NOTE: The SDK parser currently only supports legacy varint-format v1 headers,
// not CBOR-encoded v1 headers.  CBOR-format CAR verification is expected to
// fail until the SDK parser is updated.
func TestVerifyRemoteCAR_CBORFormat(t *testing.T) {
	...
	// Use a valid CID for the expected data
	...
	verified, err := VerifyRemoteCAR(context.Background(), gateway, txID, rootCID.String())
	if err == nil {
		t.Log("CBOR-format CAR v2 verification unexpectedly succeeded (SDK parser may have been updated)")
	} else {
		t.Logf("CBOR-format CAR correctly rejected (SDK parser limitation): %v", err)
	}
	_ = verified
}
...
func TestVerifyRemoteCAR_TruncatedDataSize(t *testing.T) {
	...
	if verified {
		t.Fatal("expected verified=false for truncated data_size")
	}
	t.Logf("correctly detected truncated data_size: %v", err)
}
